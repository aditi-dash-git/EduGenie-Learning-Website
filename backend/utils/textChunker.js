/**
 * Splits large PDF text into smaller pieces(chunks) so AI can process it easily.
 * @param {string} text - Full text to chunk
 * @param {number} chunkSize - Target size per chunk (in words)
 * @param {number} overlap - Number of words to overlap between chunks (overlapping words help AI to maintain context)
...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
  ...
  for (const paragraph of paragraphs) {
    // splits paragraph into words by space
...
/**
 * Finds the most relevant pieces of text based on a user query.
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  ...
  const scoredChunks = chunks.map((chunk, index) => {
    ...
    for (const word of queryWords) {
      ...
      const exactMatches = (
        // creates word boundary (e.g: word: machine then it will match only with machine , not with machines , machinery etc.)
